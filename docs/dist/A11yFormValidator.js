...
var ValidationState = class {
	...
	snapshot() {
		return {
			form: this.formState,
			fields: Object.fromEntries([...this.fieldStates.entries()].map(([name, state]) => [name, { ...state }]))
		};
	}
};
...
var A11yFormValidator = class A11yFormValidator {
	...
	collectFields() {
		...
		return [...grouped.entries()].map(([name, elements]) => {
			...
			return existing && existing.elements.length === elements.length && existing.elements.every((element, index) => element === elements[index]) ? existing : new FieldController(this, name, elements);
		});
	}
	refreshFields() {
		...
		Object.keys(this.state.snapshot().fields).forEach((name) => {
			if (!currentNames.has(name)) this.state.remove(name);
		});
	...
	handleSubmit(event) {
		...
		if (this.submissionPhase === "validating") return;
		this.submissionPhase = "validating";
		const runId = ++this.submissionRunId;
		this.processSubmission(runId);
	}
	async processSubmission(runId) {
		while (runId === this.submissionRunId && this.submissionPhase === "validating" && this.enabled && !this.destroyed) {
			const revision = this.interactionRevision;
			let valid;
			try {
				valid = await this.validate({ reason: "submit" });
			} catch (error) {
				if (runId !== this.submissionRunId || this.destroyed) return;
				this.submissionPhase = "idle";
			...
			if (runId !== this.submissionRunId || this.destroyed || !this.enabled) return;
			if (revision !== this.interactionRevision) continue;
			...
			try {
				if (typeof this.form.requestSubmit !== "function") throw new TypeError("HTMLFormElement.requestSubmit() is required for validated submission.");
				...
				if (this.submissionPhase === "resubmitting") this.submissionPhase = "idle";
			}
			return;
		}
	}
	...
	invalidateFieldValidation(field) {
	...
	restoreFieldState(field, state) {
		...
		if (state.invalid) field.setVisualState("invalid");
		else if (state.valid) field.setVisualState("valid");
		else field.clearVisualState();
	}
	invalidateFormValidation() {
		this.formValidationRunId += 1;
		if (this.getState().form === "validating") this.state.setFormState("idle");
	}
	...
	normalizeReason(reason, fallback = "manual") {
		return reason || fallback;
	}
	errorsEqual(first, second) {
		const firstFields = Object.entries(first.fields);
		const secondFields = Object.entries(second.fields);
		return first.form.length === second.form.length && first.form.every((message, index) => message === second.form[index]) && firstFields.length === secondFields.length && firstFields.every(([name, message]) => second.fields[name] === message);
	}
	emitErrorsChanged(previousErrors, source, reason) {
		const errors = this.getErrors();
		if (this.errorsEqual(previousErrors, errors)) return false;
	...
	fieldEventDetail(field, reason, state) {
		return {
			instance: this,
			validator: this,
			form: this.form,
	...
	async validate(options = {}) {
		...
		try {
			...
			this.emit(EVENTS.beforeValidate, {
				reason,
				state: this.getState()
			});
			const valid = (await Promise.all(this.fields.map((field) => this.validateFieldInternal(field, reason)))).every((result) => result.valid) && this.formErrors.length === 0;
			if (runId !== this.formValidationRunId) return valid;
			...
			if (valid) this.emit(EVENTS.formValid, {
				...detail,
				valid: true
			});
			else this.emit(EVENTS.formInvalid, {
				...detail,
				valid: false
			});
			return valid;
			...
			if (runId === this.formValidationRunId) {
				...
				this.fields.forEach((field) => {
					if (!this.getState().fields[field.name]?.pending) return;
					this.validationRuns.set(field.name, (this.validationRuns.get(field.name) || 0) + 1);
					this.restoreFieldState(field, {
	...
	async validateField(input, options = {}) {
		...
		if (result.committed) this.emitErrorsChanged(previousErrors, "client-validation", reason);
		return result.valid;
	}
	async validateFieldInternal(input, reason) {
		...
		if (ignoredReason) {
			field.lastError = "";
			this.renderer.clear(field);
			const state$1 = this.state.updateField(field.name, {
			...
			const detail$1 = this.fieldEventDetail(field, reason, state$1);
			this.emit(EVENTS.fieldIgnored, {
				...detail$1,
				ignoredReason
			});
			return {
		...
		try {
			for (const [ruleName, ruleOptions] of Object.entries(rules)) {
				...
				if (this.validationRuns.get(field.name) !== runId) return {
					valid: !field.lastError,
					committed: false
				};
				if (!result.valid) {
			...
			if (this.validationRuns.get(field.name) === runId) {
				...
				if (restoredState.invalid) field.setVisualState("invalid");
				else if (restoredState.valid) field.setVisualState("valid");
				else field.clearVisualState();
			}
			throw error;
		}
	...
	setErrors(errors = {}) {
		...
		if (Object.keys(nextErrors.fields).length || nextErrors.form.length) this.state.setFormState("invalid");
		this.emitErrorsChanged(previousErrors, "server", "server");
		if (Object.keys(nextErrors.fields).length || nextErrors.form.length) this.emit(EVENTS.formInvalid, {
			valid: false,
			reason: "server",
			errors: nextErrors,
			state: this.getState()
...
//#endregion
export { getPreferredScrollBehavior as _, EVENTS as a, initFormValidators as c, MessageResolver as d, en_default as f, ensureElementId as g, ErrorRenderer as h, DEFAULT_OPTIONS as i, ValidationState as l, EventEmitter as m, ATTRIBUTES as n, SELECTORS as o, FieldController as p, CLASSES as r, createFormValidator as s, A11yFormValidator as t, RuleRegistry as u, toSafeInteger as v };
//# sourceMappingURL=A11yFormValidator.js.map